...
if ( isset($_POST['btnAgregar']) ){
	...
	echo '<script type="text/javascript">alert("Agregado correctamente");window.location.href = "ayudas.php";</script>';
...
}elseif ( isset($_GET['idDesactivar']) )
{
	...
	echo '<script type="text/javascript">alert("ayuda desactivado correctamente")</script>';
	header("Location: ayudas.php");
...
$query_update =  "update ayudas set ayuda='".utf8_decode( htmlspecialchars( addslashes( $_POST['inpayuda'] ) ) )."',
...
$rs_update = $objeto->Buscar_Instruccion($query_update);
	...
	echo '<script type="text/javascript">alert("Modificado correctamente");window.location.href = "ayudas.php";</script>';
 	//header("Location: ayudas.php");
}

if ( $_POST["inpBu2"] != "" ) {
	$Instruccion =  "SELECT ayudas.id, concat(admin.nombres,' ',admin.apellidos), ayudas.ayuda, dep.departamento, ayudas.estatus FROM ayudas INNER JOIN dep on dep.id = ayudas.idDep inner join admin on admin.id = ayudas.admin where ayudas.ayuda like '%".$_POST["inpBu1"]."%' and ayudas.estatus = '".$_POST["inpBu2"]."'";
} else {
	$Instruccion =  "SELECT ayudas.id, concat(admin.nombres,' ',admin.apellidos), ayudas.ayuda, dep.departamento, ayudas.estatus FROM ayudas INNER JOIN dep on dep.id = ayudas.idDep inner join admin on admin.id = ayudas.admin where ayudas.ayuda like '%".$_POST["inpBu1"]."%'";
}
					...
					<div id="main">
						...
						<!-- Content -->
							<section id="content" class="main">
                                    ...
                                    <section align="center">
                                        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                                            <div class="row gtr-uniform">
                                            	...
                                            	<div class="col-6">
                                                    <input type="text" name="inpayuda" id="inpayuda" value="<?php echo $_POST["inpayuda"]; ?>" onkeypress="return longitud(this, 100) && validar(event,'ambos','áéíóú ')" placeholder="Nombre del ayuda" />
                                                </div>
											            ...
											            <tr>
															...
															<td><?php echo $renglon[7]; ?></td>
											              	<td>
											              	 ...
											              	 <button type="button"><a href="ayudas.php?idModificar=<?php echo $renglon[0]; ?>">Modificar</a></button>
								...
								$query_ayuda =  "select * from ayudas where id=".$modificar;	
								$rs_ayuda = $objeto->Buscar_Instruccion($query_ayuda);
								
								$ayuda_campo	=	trim($rs_ayuda[0]['campos']);
								$array_campo	=	explode("|",$ayuda_campo);
								$total_campo	= 	count($array_campo);
								
								$ayuda_tipo	=	trim($rs_ayuda[0]['tipo']);
								$array_tipo	=	explode("|",$ayuda_tipo);
                                    ...
                                    <section align="center">
                                        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                                            <div class="row gtr-uniform">
                                            	...
                                            	<div class="col-6">
                                                    <input type="text" name="inpayuda" id="inpayuda" value="<?php echo $rs_ayuda[0]['ayuda']; ?>" onkeypress="return longitud(this, 100) && validar(event,'ambos','áéíóú ')" placeholder="Nombre del ayuda" />
                                                </div>
                                                ...
                                                <div class="col-6">
                                                    <select name="inpDep" id="inpDep" > 
														...
														if($rs_departamento_row[0] == $rs_ayuda[0]['idDep']){$SelDepartamente="selected";}else{$SelDepartamente="";}
												...
												if ( isset($_SESSION['Campo0']) ):
												 ?>
                                                    ...
                                                    <a href="ayudas.php"><input type="button" value="Volver"></a>
                                                    </div>
